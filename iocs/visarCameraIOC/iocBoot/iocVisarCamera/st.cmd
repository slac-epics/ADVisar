...
#dbLoadRecords(	"$(TOP)/db/asynRecord.db",			"P=$(BASE_NAME),R=:_____________AsynIO,PORT=$(PORT_C),ADDR=0,IMAX=0,OMAX=0" )
...
dbLoadRecords( "$(TOP)/db/visarCameraIOC.db", "P=$(BASE_NAME),R=:,PORTC=PORT_C,PORTC2=PORT_C,PORTD=PORT_D,PORT=$(PORT),ADDR=0,TIMEOUT=1")

epicsEnvSet("EPICS_DB_INCLUDE_PATH","$(ADCORE)/db")


visarCameraConfig("$(PORT)", "PORT_D" , -1,-1)
...
dbLoadRecords("$(ADCORE)/db/NDStdArrays.template", "P=$(PREFIX),R=image1:,PORT=Image1,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT),NDARRAY_ADDR=0,TYPE=Int16,FTVL=SHORT,NELEMENTS=$(NELEMENTS)")
